 ...
 * Constants.
 */
const errors = fs.createWriteStream('results/gplookup-errors.txt', {flags: 'a'}),
      results = fs.createWriteStream('results/gplookup.txt', {flags: 'a'}),
      ALL = urls.length,
      errored = [];
...
function nextCall() {
    ...
    util.apiQuery(currl, {
        ...
        if (d.error || !d || !d.query || !d.query.usercontribs) {
            ...
            if (addr.length) {
                results.write(
                    addr.map(a => `${currl}/Special:Contribs/${a}\n`)
                        .join('')
                );
            }
            if (d['query-continue']) {
